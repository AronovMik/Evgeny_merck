# LLM Council — методология

> **Для Claude:** этот файл — инструкция для тебя. Когда пользователь произносит триггерную фразу (см. секцию «when to run»), запусти процедуру, описанную ниже. Не игнорируй и не пересказывай — **исполняй пошагово**: framing → 5 advisors параллельно → peer review параллельно → chairman synthesis → отчёт.
>
> **Как подключить в репозиторий (для пользователя):**
> 1. Положи этот файл в корень репо как `LLM_COUNCIL.md` (или в `.claude/LLM_COUNCIL.md`).
> 2. В корневой `CLAUDE.md` добавь одну строку:
>    ```
>    Когда пользователь говорит "council this", "pressure-test this", "war room this", "stress-test this", "debate this", "run the council" — следуй методологии из LLM_COUNCIL.md. Запускай 5 advisors параллельно через Task tool / sub-agents.
>    ```
> 3. Всё. Дальше пиши в issue/PR/чате: `@claude council this: <вопрос>` — и Claude развернёт совет.
>
> **Замечание про GitHub @claude:** в облачной сессии нет доступа к локальным skills. Этот файл и строка в CLAUDE.md заменяют скилл — Claude прочитает их как обычные репозиторные инструкции.

---

## triggers (когда запускать совет)

**MANDATORY TRIGGERS** (запускать всегда): `council this`, `run the council`, `war room this`, `pressure-test this`, `stress-test this`, `debate this`.

**STRONG TRIGGERS** (запускать только при реальном trade-off): `should I X or Y`, `which option`, `what would you do`, `is this the right move`, `validate this`, `get multiple perspectives`, `I can't decide`, `I'm torn between`.

**Do NOT trigger** на простые yes/no, фактические лукапы, casual `should I` без значимого trade-off (например `should I use markdown` — не для совета).
