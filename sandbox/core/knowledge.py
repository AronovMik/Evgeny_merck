...
ЧТО ВЗЯТО ИЗ ДОКУМЕНТАЦИИ LANGDOCK (docs.langdock.com, разделы Knowledge
Basics, Document Search, Projects). Это утверждения платформы о себе, а не
пересказ того, что написано в промпте тренажёра:

  * «Langdock uses context injection, where relevant content is sent to the AI
    along with your prompt» — найденное подставляется в контекст ДО вызова
    модели, а не запрашивается моделью через инструмент;
  * система сама выбирает между двумя способами: небольшие документы идут
    в окно контекста целиком, крупные — через поиск по эмбеддингам;
  * «Up to 50 chunks are retrieved per query»;
  * файлы, приложенные к агенту, подтягиваются автоматически при ответе.
...
ЧЕГО МЫ НЕ ЗНАЕМ и где возможно расхождение с платформой:
  ...
  1. Порог, с которого файл перестаёт подаваться целиком. У нас 4000 симв.
  2. Размер фрагмента, перекрытие, модель эмбеддингов. У нас 1200 симв.,
     перекрытие 150, text-embedding-3-small на 512 измерений.
  3. Куда именно платформа кладёт найденное в списке сообщений. У нас —
     отдельным системным сообщением после инструкций.
  4. Общий это индекс по всем файлам или отдельный на файл; относится ли
     потолок в 50 фрагментов к запросу целиком или к каждому файлу.
...
EMBEDDING_DIMENSIONS = 512  # укорочённый вектор: индекс легче, качество почти то же
