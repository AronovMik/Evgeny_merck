...
По документации Langdock файлы проекта попадают в модель так:

  * контент, слишком большой, чтобы отдать целиком, идёт через семантический
    поиск: файл режется на фрагменты, фрагменты превращаются в эмбеддинги,
    по запросу выполняется векторный поиск и подставляются наиболее
    релевантные куски — **до 50 фрагментов на запрос**;
  * небольшие вложения могут попадать в контекст более полно, когда важна
    точная формулировка.
...
ЧЕСТНАЯ ГРАНИЦА. Langdock не публикует свой размер фрагмента, модель
эмбеддингов и место вставки найденного в промпт. Поэтому здесь повторяется
МЕХАНИЗМ, а не побайтовое поведение платформы. Все параметры повтора
(модель эмбеддингов, размер фрагмента, сколько подставлено) пишутся в лог
прогона, и расхождение с платформой фиксируется как отклонение — чтобы
разница была видна, а не подразумевалась.
"""
